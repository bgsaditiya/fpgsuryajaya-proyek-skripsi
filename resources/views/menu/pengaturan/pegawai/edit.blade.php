...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <a class="btn" id="tombol-putih" href="{{ route('pegawai') }}">Kembali</a>
        </div>
        <div class="card-body">
            <form method="post" action="{{ route('pegawai.update', $username) }}">
                @csrf
                @method('PUT')
                <div class="row mb-3">
                ...
                <button type="submit" id="tombol" class="btn float-end">Simpan</button>
            </form>
        </div>
    </div>
@endsection
